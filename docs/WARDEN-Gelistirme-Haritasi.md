...
## 7. Milestone 6 — Faz 3 Önizlemesi: Dinamik Analiz (İsteğe Bağlı, Zaman Kalırsa)

Bu milestone, mimari raporun 5.3.7 bölümünde tanımlanan **stretch-goal** modülleri içindir. Faz 1-2 (Milestone 0-5) zaten kendi başına eksiksiz bir bitirme projesi oluşturduğundan, bu adımlara yalnızca zaman kalırsa geçmen önerilir — yarım kalmış bir dinamik analiz modülünden çok, eksiksiz çalışan bir Faz 1-2 daha değerlidir.

**Adım 6.1 — Docker sandbox'ı elle dene**
Kod yazmadan önce, terminalde basit bir Python betiğini `docker run --network=none --memory=128m --read-only ...` kısıtlarıyla manuel çalıştır.
✅ *Bittiğinde göreceğin:* Kısıtlı bir konteynerin ağa erişemediğini, disk yazamadığını elinle doğruluyorsun — bir sonraki adımda bunu otomatikleştireceksin.

**Adım 6.2 — SandboxExecutor iskeletini yaz**
Bir Python fonksiyonunu ve test girdisini alıp, yukarıdaki kısıtlarla bir Docker konteynerinde çalıştıran, çıktı/hata/çökme bilgisini toplayan basit bir servis yaz.
✅ *Bittiğinde göreceğin:* Kasıtlı olarak çöken bir fonksiyon için "crashed: ZeroDivisionError" gibi bir sonuç dönüyor.

**Adım 6.3 — AITestGenerator'ın ilk basit versiyonunu yaz**
Bir fonksiyonun imzasını LLM'e verip 3-5 sınır-durum test girdisi ürettir (henüz karmaşık değil, tek bir prompt yeterli).
✅ *Bittiğinde göreceğin:* Basit bir `divide(a, b)` fonksiyonu için LLM'in "b=0 dene" gibi anlamlı bir senaryo ürettiğini görüyorsun.

**Adım 6.4 — İki modülü birleştir**
AITestGenerator'ın ürettiği girdileri SandboxExecutor'da çalıştır, sonuçları raporla.
✅ *Bittiğinde göreceğin:* Uçtan uca "fonksiyonu AI için anlamlı testler üret → izole ortamda çalıştır → sonucu göster" akışı çalışıyor — bu noktada BehavioralDiffEngine'i eklemek (aynı girdilerle eski/yeni kod versiyonunu karşılaştırmak) küçük bir ek adım olur, isterse sen devam edebilirsin.

---

## 8. Milestone 7 — Cilalama ve Bitirme Hazırlığı

**Adım 7.1 — Test kapsamını artır**
...
**Adım 7.2 — Gerçek projelerinde pilot kullanım**
...
**Adım 7.3 — README ve kurulum dokümantasyonu yaz**
...
**Adım 7.4 — Demo senaryosu prova et**
Sunum için: (1) açık bir SQL injection'ı yakalama, (2) hayali bir paketi engelleme, (3) Claude Code'un aracı kendiliğinden çağırması, (4) tehlikeli bir ajan eylemini gerçekleşmeden önce durdurma — dört senaryoyu birkaç kez prova et.
✅ *Bittiğinde göreceğin:* Sunumda hiç aksama olmadan senaryoları art arda gösterebiliyorsun.
...
| 5 | MCP Sunucusu + Agent Eylem İzleyici | 6-7 gün |
| 6 | Faz 3 Önizlemesi: Dinamik Analiz (isteğe bağlı, zaman kalırsa) | 5-7 gün |
| 7 | Cilalama + Demo Provası | 5-6 gün |

Milestone 3'ün sonunda elinde zaten **çalışan, uçtan uca bir sistem** olacak (editörsüz, sadece Git hook ile) — bu senin için hem motive edici bir kilometre taşı hem de "en kötü ihtimalle bu bile yeterli bir bitirme projesi" güvencesi. Milestone 6 (dinamik analiz) tamamen isteğe bağlıdır — Milestone 0-5+7 zaten eksiksiz, savunulabilir bir bitirme projesi oluşturur; zaman kalmazsa Milestone 6'yı atlayıp doğrudan 7'ye geçebilirsin.
